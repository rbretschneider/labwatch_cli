...
# Marker comment identifying labwatch-managed systemd unit files
MANAGED_MARKER = "# labwatch:managed"


def _unit_header() -> str:
    """Return a comment header for a labwatch-generated systemd unit file."""
    return (
        f"# Generated by labwatch mount-builder — do not edit manually.\n"
        f"# To reconfigure, run: labwatch mount-builder\n"
        f"{MANAGED_MARKER}\n"
    )
...
def detect_managed_units() -> List[str]:
    """Return paths of labwatch-managed systemd unit files."""
    systemd_dir = Path("/etc/systemd/system")
    if not systemd_dir.is_dir():
        return []
    results = []
    # Scan mount units, drop-in overrides, and docker overrides
    patterns = ["mnt-*.mount", "mnt-*.mount.d/override.conf",
                "docker.service.d/override.conf"]
    for pattern in patterns:
        for p in systemd_dir.glob(pattern):
            try:
                if MANAGED_MARKER in p.read_text():
